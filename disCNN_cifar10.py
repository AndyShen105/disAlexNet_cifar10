...
if FLAGS.job_name == "ps":
	...
	step = 1
	count = 0
	result_data = open("/root/result/result_pic7.csv", "a+")
	while (not sv.should_stop()) and (step <= n_batches_per_epoch * Epoch and cost>=targted_loss):
		...
		process_data = open("/root/result/process_data7.csv", "a+")
		line = str(ith_epoch)+","+str(n_PS) + ',' + str(n_Workers) +",1,1,6,"+ str(batch_size) + ','+str(learning_rate)+','+str(n_intra_threads)+',' +str(n_inter_threads)+","+ str(cost) + ',' + str(Epoch_Time)
        ...
        result = str(n_PS) + ',' + str(n_Workers) +",1,1,6," + str(batch_size) + ','+str(learning_rate)+','+str(n_intra_threads)+',' +str(n_inter_threads)+ ","+ str(cost) + ',' + str(total_time)
